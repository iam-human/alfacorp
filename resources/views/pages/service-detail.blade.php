        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div>
                <div class="bg-surface-alt rounded-2xl p-8 border border-slate-100 sticky top-28">
                    <h3 class="text-xl font-heading font-bold mb-6 text-slate-900 border-b border-border pb-4">Layanan Lainnya</h3>
                    <ul class="space-y-4">
                        @foreach($relatedServices as $related)
                            <li>
                                <a href="{{ route('service.detail', $related->slug) }}" class="flex items-center group">
                                    <div class="w-10 h-10 bg-white text-primary rounded-lg flex items-center justify-center mr-4 group-hover:bg-primary group-hover:text-white transition-colors border border-slate-100">
                                        @if($related->icon)
                                            @svg($related->icon, 'w-5 h-5')
                                        @endif
                                    </div>
                                    <span class="font-medium text-neutral-600 group-hover:text-primary transition-colors">{{ $related->title }}</span>
                                </a>
                            </li>
                        @endforeach
                    </ul>
                    ...
                    <div class="mt-8 pt-8 border-t border-border">
                        <h3 class="font-bold text-slate-900 mb-4">Tertarik dengan layanan ini?</h3>
